...
- We have `r format(block_count[1], big.mark = ",")` blocks of `r block_size[1]` elements, `r format(block_count[2], big.mark = ",")` blocks of `r block_size[2]` elements,..., `r format(block_count[length(block_count)], big.mark = ",")` blocks of `r block_size[length(block_size)]` elements.
...
For example, our approach results in a `r sprintf("%.2f", (result_2_reclin$metrics)[4]*100)`% false negative rate (FNR). To improve this, we can increase the `epsilon` parameter of the NND method from 0.1 to 0.2. To do so, we configure the `control_ann` parameter in the blocking function using the `controls_ann` and `control_nnd` functions. 

```{r reclin_nnd_improved}
