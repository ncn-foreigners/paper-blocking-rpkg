...
\tightlist
\item
  create shingles via the \CRANpkg{tokenizers} \citep{tokenizers} and \CRANpkg{text2vec} \citep{text2vec} packages or a matrix of vectors (e.g.~embeddings via the \CRANpkg{ragnar} \citep{ragnar} package).
\item
  search for nearest neighbours using approximate algorithms implemented in the \CRANpkg{rnndescent} \citep{rnndescent}, \CRANpkg{RcppHNSW} \citep{RcppHNSW}, \CRANpkg{mlpack} \citep[\citet{mlpack2025}]{mlpack2023}, and \CRANpkg{RcppAnnoy} \citep{RcppAnnoy}.
\item
  create blocks using \CRANpkg{igraph} \citep[\citet{igraph2006}]{igraph2025}.
\end{enumerate}

This is the solely package in the R ecosystem that allows to easily apply modern ANN algorithms and significantly speed-up the record linkage / deduplication problems. In addition, we have developed the \texttt{pair\_ann()} function to seamless integrate with the \CRANpkg{reclin2} package.
...
\item
  \texttt{ann} -- what ANN algorithm should be applied, by default we use the \CRANpkg{rnndescent} package as it allows supports sparse matrices,
\item
...
Users interested in integration with the \CRANpkg{reclin2} package we refer to
