...
we provide description of the main functionalities of the `blocking`
...
-   `x, y` -- reference vectors, where `y = NULL` which indicate that
...
-   `representation` -- whether `x` and `y` should be represented as
    ...
    argument),
-   `ann` -- what ANN algorithm should be applied, by default we use the
...
-   `control_ann` -- controls provided in the `controls_ann()` allows
...
This function return an object of the `blocking` class with the
following elements:
...
Let's examine the block. Obviously, there are typos in the `fname`
and `surname`. Nevertheless, all record refer to the same entity (as denoted by `true_id`).
