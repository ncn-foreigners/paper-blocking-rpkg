...
languages (cf. @Phonetic2020) to block records that sound similar but
...
-   **Phonetic blocking**:
    ...
    -   Additionally, \CRANpkg{stringdist} [@stringdist] implements the
        Soundex algorithm, whilst \CRANpkg{phonics} (@phonics,
        @Phonetic2020) implements various phonetic algorithms that can
...
cf. @johndrow2018theoretical). The basic workflow of the
...
1.  Create shingles of the input character vectors using
    ...
    character vectors.
2.  Search for nearest neighbours using ANNs algorithms implemented in
...
-   `representation` -- whether `x` and `y` should be represented as
    ...
    argument);
-   `ann` -- which ANNs algorithm should be applied (by default, we use
...
[@Dong2011] implemented in the \CRANpkg{rnndescent} package.
...
the random seed (`t: 1232` denotes how many 2-character shingles were
created).
...
```{r penguins-alison, out.width = "80%", out.height = "30%", fig.cap = "Distribution of distances between true matches and non-matches within blocks", fig.alt="A density plot of distances between units that are true matches (red) and non-matches (blue) within blocks created by the \\CRANpkg{blocking}. The distribution of distance for matches is bimodal. There is a group of units that are true matches where the distance between them is small (less than 0.2), whilst for the second group, the distance is similar to true non-matches (between 0.4 and 0.6). This distance may be used as additional information for deduplication (and record linkage) studies."}
