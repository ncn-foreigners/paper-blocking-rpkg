...
embeddings [@mikolov2013efficient] to search for the closest neighbour
...
@johndrow2018theoretical). The basic idea behind the \pkg{blocking}
...
2.  search for nearest neighbours using ANNs
...
`pair_ann()` function to seamlessly integrate with the \pkg{reclin2}
package which is described in one of the package vignettes.

## Outline of article

The paper has the following structure. In the Section 2
we provide description of the main functionalities of the \pkg{blocking}
...
-   `ann` -- which ANN algorithm should be applied, by default we use the
    \pkg{rnndescent} package as it supports sparse matrices,
...
-   `control_ann` -- controls provided in the `controls_ann()` allow
    ...
    referring to a specific algorithm, e.g., `control_nnd()` for the NND
    algorithm).
...
columns (excluding `true_id`) in each row. Information stored in the `txt` 
column will be used  for blocking records in the `blocking()` function. 
...
the \pkg{rnndescent} package. Additionally, we set `verbose = 1` to monitor progress. Note
...
`result_reclin` object. In thus example we have created
...
`r sprintf("%.2f", (res_reclin$metrics)[4]*100)`% FNR. 
To improve this, we can increase the `epsilon` parameter of
...
Below we present an example using the `reclin2` package using a simple score.
...
In this section we demonstrate deduplication application using the `blocking()` function on the
`RLdata500` dataset from the \pkg{RecordLinkage} package. Note that
...
For the purpose of this example we create a new column (`id_count`) that indicates 
...
Navigable Small World (HNSW; @malkov2018efficient) algorithm from the \pkg{RcppHNSW}
package.
...
supported by the `blocking` function, i.e. NND, HNSW, Annoy (from the \pkg{RcppAnnoy} package),
Locality-Sensitive Hashing (LSH, from the \pkg{mlpack} package), and
k-Nearest Neighbours (kNN -- denoted as `"kd"`, from the \pkg{mlpack}
...
We compare our package with the `klsh()`
function from the \pkg{klsh} package, configured to create 10 blocks (denoted as `klsh_10`)
and 100 blocks (denoted as `klsh_100`), respectively. In both settings, we use 20 random
...
In contrast, the LSH-based methods show more variable performance: the `mlpack` LSH implementation achieves 98% recall but suffers from higher FPR (3.74%) and importantly FNR (2%), while the `klsh` package results reveal a trade-off between block granularity and performance -- `klsh_10` with only 10 blocks shows poor recall (84%), high FNR (10.13%) and FNR (16%), whereas `klsh_100` with 100 blocks recovers much of the performance (90% recall, 0.94% FPR but high FNR 10%). This indicate that the `klsh`, in particular these implementations of the LSH approach, miss large number of true matches.
...
\pkg{blocking} package. We believe that the software will be useful
...
Furthermore, users interested in integration with the \pkg{reclin2} package we
refer to the documentation of the `pair_ann()` function and the vignette
entitled
[`"Integration with existing packages"`](https://cran.r-project.org/web/packages/blocking/vignettes/v3-integration.html) which provides cases studies how the \pkg{blocking} package can be included in the existing record linkage / deduplication pipelines.
...
ANNs algorithms (e.g. FAISS) or usage of embeddings. For more details
