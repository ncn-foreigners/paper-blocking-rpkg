...
analyses based on multiple sources (cf.~\citet{fellegi1969theory},
\citet{Binette2022}). The goal is to link records that refer to the same entity
...
resource constraints and clerical review workload (cf.~\citet{christen2012book})
...
such as Soundex (cf.~\citet{knuth1973}) or its adaptations for non-English
languages (cf.~\citet{Howard2020}) to block records that sound similar but are
...
methods, see \citet{Christen2012}, \citet{Steorts2014} or \citet{Papadakis2020}. Section
\hyperref[sec-software]{1.2} discusses existing R packages that implement
blocking methods.
...
\item
  ...
  \item
    \CRANpkg{RecordLinkage} (\citet{RecordLinkage}, \citet{RecordLinkage-rjournal}) allows specifying blocking variables in
  ...
  \item
    \CRANpkg{PPRL} (\citet{PPRL}) focuses on privacy preserving
...
substantially faster (cf. \citet{indyk1998}, \citet{wang2021comprehensive}),
making them naturally suited to noisy or incomplete data. This approach
is similar to micro-clustering (cf. \citet{johndrow2018theoretical}), but we do
...
\item
  ...
  static or dynamic). An example using embeddings is provided in Supplementary Materials.
\item
  ...
  cf. \citet{rnndescent}), \CRANpkg{RcppHNSW} \citep{RcppHNSW},
  \CRANpkg{mlpack} (\citet{mlpack2023}, \citet{mlpack2025}), and \CRANpkg{RcppAnnoy}
  \citep{RcppAnnoy}.
\item
  Create final blocks using \CRANpkg{igraph} (\citet{igraph2025},
  \citet{igraph2006}).
\end{enumerate}
...
results. The first is the \emph{reduction ratio} (RR, cf. \citet{christen2007quality}), which indicates the
...
Metrics calculated based on this confusion matrix are defined in Table
\ref{tab:eval-metrics}. In the context of blocking, these metrics are
...
\item
  F1 Score: Combines precision and recall. However, it may be less relevant in the context of blocking, since both low precision and low recall are expected. For a~critique of the F1 score in the context of record linkage see \citet{hand2018}, and for an alternative F* measure see \citet{hand2021} (not yet implemented).
\end{itemize}
...
columns (excluding \texttt{true\_id}) in each row. In computer science, this is called schema-agnostic blocking (cf.~\citet{papadakis2016schema}). Information stored in the
...
\begin{figure}[!ht]
\includegraphics[width=1\linewidth,alt={A density plot of distances between units that are true matches (red) and non-matches (blue) within blocks created by the `blocking`. The distribution of distance for matches is bimodal. There is a group of units that are true matches where the distance between them is small (less than 0.2), while for the second group, the distance is similar to true non-matches (between 0.4 and 0.6). This distance may be used as additional information for deduplication (and record linkage) studies.}]{paper-blocking_files/figure-latex/density-plot-1} \caption{Distribution of distances between true matches and non-matches within blocks}\label{fig:density-plot}
\end{figure}
...
\hline
nnd & 5.17 & 0.74 & 0 & 99.26 & 9.83\\
\hline
hnsw & 4.76 & 0.80 & 0 & 99.20 & 9.08\\
\hline
...
Finally, to extend the package's ability to block large datasets, we could allow users to use DuckDB \citep{raasveldt2019duckdb} via the \CRANpkg{duckdb} package \citep{duckdb}. DuckDB smoothly handles large datasets and allows for extensions that implement ANN algorithms (see \url{https://duckdb.org/community_extensions/extensions}\}.

\section{Acknowledgements}\label{acknowledgements}

We thank the three reviewers for their comments. We also thank participants of the uRos 2024 conference for valuable comments and discussion. Work on this package was supported by the National Science Centre, OPUS 20 grant no.~2020/39/B/HS4/00941.
