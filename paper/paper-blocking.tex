...
methods, see \citet{Steorts2014} or \citet{Papadakis2020}. Section \hyperref[sec-software]{1.2} discusses
...
This paper is structured as follows. Section \hyperref[sec-blocks]{2} provides a description of
the main functionalities of the \CRANpkg{blocking} package and how
results can be evaluated. Section \hyperref[sec-case]{3} presents two case studies:
...
The \texttt{blocking()} function also returns recall (sensitivity, or pairs completeness) and specificity,
...
\begin{figure}[!ht]
\includegraphics[width=1\linewidth,alt={A density plot of distances between units that are true matches (red) and non-matches (blue) within blocks created by the `blocking`. The distribution of distance for matches is bimodal. There is a group of units that are true matches where the distance between them is small (less than 0.2), whilst for the second group, the distance is similar to true non-matches (between 0.4 and 0.6). This distance may be used as additional information for deduplication (and record linkage) studies.}]{paper-blocking_files/figure-latex/density-plot-1} \caption{Distribution of distances between true matches and non-matches within blocks}\label{fig:density-plot}
\end{figure}
...
results are presented in Table \ref{tab:table-latex}, and the code generating it
...
\texttt{klsh\_10} with only 10 blocks shows high FNR (16\%) and FPR
(10.13\%), whereas \texttt{klsh\_100} with 100 blocks recovers
